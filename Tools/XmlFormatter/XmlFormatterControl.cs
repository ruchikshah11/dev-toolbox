...
{
    /// <summary>
    /// A side-by-side XML formatter: your pasted/uploaded input on the left, the formatted
    ...
    /// </summary>
    public class XmlFormatterControl : UserControl
    {
        // RichTextBox, not TextBox, so the input can be colorized as XML markup - see the
        // TextChanged wiring in BuildSplitView.
        private readonly RichTextBox _txtInput = new();
        private readonly XmlOutputView _output = new();
        ...
        public XmlFormatterControl()
        {
            ...
            var card = CardPanel.Add(this, "XML Formatter - your input on the left, formatted output on the right", 0, fill: true);
            BuildSplitView(card);
        ...
        private void TryFormat()
        {
            try
            {
                ...
                var formatted = XmlFormatterService.Format(_txtInput.Text, indentStyle);

                // Reparses the same text XmlFormatterService just validated - cheap for the sizes
                // this tool handles, and keeps the Tree View tab in sync without threading the
                // parsed XDocument through the service's return type.
                var root = XDocument.Parse(_txtInput.Text).Root;
                _output.Render(formatted, root);
                HideError();
            }
        ...
        private sealed class IndentOption
        {
            ...
            {
                new("2 spaces per indent level", XmlIndentStyle.TwoSpaces),
                new("3 spaces per indent level", XmlIndentStyle.ThreeSpaces),
                new("4 spaces per indent level", XmlIndentStyle.FourSpaces),
                new("Tab delimited", XmlIndentStyle.Tab),
                new("Compact (1 line)", XmlIndentStyle.Compact),
            };

            private IndentOption(string display, XmlIndentStyle style)
            {
                Display = display;
                Style = style;
            }
            ...
            public string Display { get; }
            public XmlIndentStyle Style { get; }
            public override string ToString() => Display;
        }
    }
}
