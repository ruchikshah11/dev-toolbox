...
{
    /// <summary>
    /// File-based tool (no paste box - .pdf is a binary format): pick a PDF, convert it, then
    /// save the extracted-text .md file anywhere via a Save As dialog.
    /// </summary>
    public class PdfToMarkdownControl : UserControl
    {
        ...
        public PdfToMarkdownControl()
        {
            Dock = DockStyle.Fill;
            BackColor = Theme.Background;

            // Same-edge Dock=Top siblings stack in reverse add-order (see PdfToWordControl) - the
            // action card is added first so the source-file card (added last) ends up visually
            // on top of it.
            BuildActionCard();
            BuildSourceCard();
        }
        ...
        private void BuildActionCard()
        {
            ...
            _btnConvert.Text = "Convert to Markdown & Save As...";
            _btnConvert.UseMnemonic = false;
            _btnConvert.Location = new Point(18, 44);
            _btnConvert.Size = new Size(240, 32);
            ...
            {
                Text = "Extracts page text as plain paragraphs, separated by a horizontal rule between "
                       + "pages. Fonts, headings, columns, tables, images, and scanned (image-only) "
        ...
        private void OnConvertClick(object? sender, EventArgs e)
        {
            ...
            {
                Title = "Save converted Markdown file as",
                FileName = Path.GetFileNameWithoutExtension(_sourceFilePath) + ".md",
            ...
            try
            {
                PdfToMarkdownService.Convert(_sourceFilePath, dialog.FileName);
                ShowStatus($"Saved Markdown file to {dialog.FileName}", isError: false);
            }
            catch (Exception ex)
            {
