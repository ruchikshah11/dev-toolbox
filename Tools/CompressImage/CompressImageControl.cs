...
{
    /// <summary>
    /// File-based tool (no paste box - images are a binary format): pick an image, choose a
    /// compression preset, then save the recompressed JPEG via a Save As dialog. See
    /// <see cref="CompressImageService"/> for why the output is always JPEG.
    /// </summary>
    public class CompressImageControl : UserControl
    {
        ...
        /// <summary>Builds the "choose an image" card with its file-name/size readout.</summary>
        private void BuildSourceCard()
        {
            var card = CardPanel.Add(this, "IMAGE FILE", 100);
        ...
        /// <summary>Builds the compression-level radio buttons, the Compress button, the format note, and the status label.</summary>
        private void BuildActionCard()
        {
            ...
            _lblNote.Text = "Output is always a JPEG - a transparent PNG/GIF is flattened onto a white background "
                + "first, since JPEG has no alpha channel.";
            ...
            _lblStatus.Size = new Size(card.Width - 36, 40);
        ...
        /// <summary>Opens a file picker for the source image.</summary>
        private void OnChooseFileClick(object? sender, EventArgs e)
        {
            using var dialog = new OpenFileDialog
            {
                Title = "Choose an image file",
                Filter = "Image files (*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.tif;*.tiff)|*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.tif;*.tiff|All files (*.*)|*.*"
            };
        ...
        private void OnCompressClick(object? sender, EventArgs e)
        {
            ...
            {
                ShowStatus("Choose an image file first.", isError: true);
                return;
            }

            var level = _radLow.Checked ? ImageCompressionLevel.LowCompressionHighQuality
                : _radHigh.Checked ? ImageCompressionLevel.HighCompressionLowQuality
                : ImageCompressionLevel.MediumCompression;

            using var dialog = new SaveFileDialog
            {
                Title = "Save compressed image as",
                FileName = Path.GetFileNameWithoutExtension(_sourceFilePath) + "-compressed.jpg",
            ...
            try
            {
                var result = CompressImageService.Compress(_sourceFilePath, dialog.FileName, level);
                ...
                ShowStatus(
                    ...
                    + $"({percent:0.#}% smaller) to {dialog.FileName}",
                    isError: false);
            }
            catch (Exception ex)
            {
